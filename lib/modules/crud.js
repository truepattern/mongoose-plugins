...
exports.crudHelpers = function(schema, options) {
  ...
  schema.methods.toJSON = function (options) {
    ...
    var refobjs = _.filter(_.keys(schema.paths), function(field) {
      ...
      return field.charAt(0)!='_' && path.instance=='ObjectID' && 
        (!path.options.aonx || (path.options.aonx && path.options.aonx.inline));
    });
  ...
  schema.statics.index = function(req,res) {
    ...
    // Lets populate here all the object references as well
    var refobjs = this.objReferences();
  ...
  /**
   * Lookup any ObjectId's that needs to be drilled down.
   * Any object with _ prefix is untouched (typically internal)
   */
  schema.statics.objReferences = function() {
    return _.filter(_.keys(schema.paths), function(field) {
      var path=schema.paths[field];
      return field.charAt(0)!='_' && path.instance=='ObjectID' && 
        (!path.options.aonx || (path.options.aonx && path.options.aonx.inline));
    });
  ...
  schema.statics.updateParams = function(params, record, cb) {
    ...
    async.forEachSeries(refobjs, function(refobj, next) {
      ...
      model.updateParams(modelParams, r, function(err, modelParams, obj) {
        if(!_.isEqual(modelParams, params) && obj) {
      ...
      if(err) {
        ...
        record.save(function(err) { 
          if(err) {
            ...
            self.loadFullObject(record._id, cb);
          }
        });
      }
    });
