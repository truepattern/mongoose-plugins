...
exports.installUserSchema = function(mongoose, everyauth, mongooseAuth, options) {
  ...
  // install some checks
  /*
   * If e-mail id is not set, then set the dummy e-mail id as lastname.firstname 
   * and set active to be false
   */
  UserSchema.pre('save', function(next) {
    if(!this.email) {
      var tmpmailid = this.lastname + '.' + this.firstname;
      this.email = tmpmailid;
      this.active = false;
    }
    next();
  });
